...
test('simple variable find', () =>  {
    let r = s.findInScope(s.State(null, { foo: a.number(1) }), 'foo');
    expect(r).toEqual(a.number(1));
});

test('simple variable find failure', () =>  {
    let r = s.findInScope(s.State(null, { foo: a.number(1) }), 'notFoo');
    expect(r).toEqual(null);
});

test('variable find more than one state', () =>  {
    let r = s.findInScope(s.State(s.State(null, { bar: a.number(2) }), { foo: a.number(1) }), 'bar');
    expect(r).toEqual(a.number(2));
});

test('variable find first occurence', () =>  {
    let r = s.findInScope(s.State(s.State(null, { foo: a.number(2) }), { foo: a.number(1) }), 'foo');
    expect(r).toEqual(a.number(1));
});

test('variable find deep state', () =>  {
    let r = s.findInScope(s.State(s.State(s.State(s.State(s.State(s.State(s.State(s.State(null, {}), { foo: a.number(1)}), {}), {}), {}), {}), {}), {}), 'foo');
    expect(r).toEqual(a.number(1));
});
...
test('simple variable set non-existing', () =>  {
    ...
    expect(state.value().foo).toEqual(a.number(2));
});

test('simple variable set existing', () =>  {
    const state = s.State(null, { foo: a.number(1) });
    s.setVariable(state, 'foo', a.number(2));
    expect(state.value().foo).toEqual(a.number(2));
});
...
test('variable set more than one state', () =>  {
    ...
    expect(state.parent().value().foo).toEqual(undefined);
    // Only current state is updated.
    expect(state.value().foo).toEqual(a.number(1));
});

test('variable set first occurence', () =>  {
    const state = s.State(s.State(null, { foo: a.number(1) }), { foo: a.number(1) });
    s.setVariable(state, 'foo', a.number(2));
    // parent state 'foo' value is unchanged.
    expect(state.parent().value().foo).toEqual(a.number(1));
    // current state 'foo' is changed.
    expect(state.value().foo).toEqual(a.number(2));
});

test('variable set deep state', () =>  {
    const state = s.State(s.State(s.State(s.State(s.State(s.State(s.State(s.State(null, {}), { foo: a.number(1)}), {}), {}), {}), {}), {}), {});
    s.setVariable(state, 'foo', a.number(2));
    expect(state.parent().parent().parent().parent().parent().parent().value().foo).toEqual(a.number(2));
});
...
test('closure creation', () => {
    const parent = s.State(null, { foo: a.number(1)});
